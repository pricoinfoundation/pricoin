...
static RPCMethod validateaddress()
{
    return RPCMethod{
        "validateaddress",
        "Return information about the given address. Recognises both transparent\n"
        "Bitcoin-shaped addresses and Pricoin's stealth encodings (master and\n"
        "subaddress). The `kind` field tells callers which family the address\n"
        "belongs to so withdrawal flows can route correctly.\n",
        {
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::STR, "kind", /*optional=*/true,
                    "Address family: \"transparent\" | \"stealth_main\" | \"stealth_subaddress\". "
                    "Absent on invalid input."},
                {RPCResult::Type::STR, "address", /*optional=*/true, "The address validated"},
                {RPCResult::Type::STR_HEX, "scriptPubKey", /*optional=*/true,
                    "Hex-encoded output script. Transparent only — stealth addresses don't "
                    "have a one-shot scriptPubKey (the sender derives a fresh per-output P)."},
                {RPCResult::Type::BOOL, "isscript", /*optional=*/true, "If the key is a script (transparent only)"},
                {RPCResult::Type::BOOL, "iswitness", /*optional=*/true, "If the address is a witness address (transparent only)"},
                {RPCResult::Type::NUM, "witness_version", /*optional=*/true, "The version number of the witness program"},
                {RPCResult::Type::STR_HEX, "witness_program", /*optional=*/true, "The hex value of the witness program"},
                {RPCResult::Type::STR_HEX, "view_pubkey", /*optional=*/true,
                    "Public view point (33 bytes). Stealth addresses only."},
                {RPCResult::Type::STR_HEX, "spend_pubkey", /*optional=*/true,
                    "Public spend point (33 bytes). Stealth addresses only."},
                {RPCResult::Type::NUM, "subaddress_index", /*optional=*/true,
                    "Subaddress index (≥1). Subaddress encodings only."},
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            // Try Pricoin stealth/subaddress first. The two families are
            // disjoint (different version bytes, different payload sizes)
            // so neither will misclassify a valid transparent encoding.
            if (auto parsed = ::pricoin::stealth::ParseStealthAddress(addr_str)) {
                UniValue ret(UniValue::VOBJ);
                ret.pushKV("isvalid", true);
                ret.pushKV("kind", parsed->kind == ::pricoin::stealth::AddressKind::Subaddress
                                    ? "stealth_subaddress" : "stealth_main");
                ret.pushKV("address", addr_str);
                ret.pushKV("view_pubkey",  HexStr(parsed->address.view));
                ret.pushKV("spend_pubkey", HexStr(parsed->address.spend));
                if (parsed->kind == ::pricoin::stealth::AddressKind::Subaddress) {
                    ret.pushKV("subaddress_index", (uint64_t)parsed->subaddress_index);
                }
                return ret;
            }
            ...
            CTxDestination dest = DecodeDestination(addr_str, error_msg, &error_locations);
