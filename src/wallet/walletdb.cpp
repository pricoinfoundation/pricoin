...
const std::string PRICOIN_ADAPTOR_SWAP{"pct_adaptor_swap"};
...
bool WalletBatch::WritePricoinStealthSeed(const std::vector<unsigned char>& blob)
{
    return WriteIC(DBKeys::PRICOIN_STEALTH_SEED, blob);
}

bool WalletBatch::ReadPricoinStealthSeed(std::vector<unsigned char>& blob)
{
    return m_batch->Read(DBKeys::PRICOIN_STEALTH_SEED, blob);
}
...
bool WalletBatch::WritePricoinSwapSession(const uint256& session_id,
                                          const std::vector<unsigned char>& blob)
{
    return WriteIC(std::make_pair(DBKeys::PRICOIN_SWAP_SESSION, session_id), blob);
}

bool WalletBatch::ErasePricoinSwapSession(const uint256& session_id)
{
    return EraseIC(std::make_pair(DBKeys::PRICOIN_SWAP_SESSION, session_id));
}

bool WalletBatch::ReadAllPricoinSwapSessions(
    std::map<uint256, std::vector<unsigned char>>& out)
{
    DataStream prefix;
    prefix << DBKeys::PRICOIN_SWAP_SESSION;
    std::unique_ptr<DatabaseCursor> cursor = m_batch->GetNewPrefixCursor(prefix);
    if (!cursor) return false;
    while (true) {
        DataStream ssKey;
        ...
        std::string type;
        ssKey >> type;
        if (type != DBKeys::PRICOIN_SWAP_SESSION) continue;
        uint256 session_id;
        ssKey >> session_id;
        std::vector<unsigned char> blob;
        ssValue >> blob;
        out.emplace(session_id, std::move(blob));
    }
    return true;
}

bool WalletBatch::WritePricoinSwapCeremony(const uint256& ceremony_id,
                                            const std::vector<unsigned char>& blob)
{
    return WriteIC(std::make_pair(DBKeys::PRICOIN_SWAP_CEREMONY, ceremony_id), blob);
}

bool WalletBatch::ErasePricoinSwapCeremony(const uint256& ceremony_id)
{
    return EraseIC(std::make_pair(DBKeys::PRICOIN_SWAP_CEREMONY, ceremony_id));
}

bool WalletBatch::ReadAllPricoinSwapCeremonies(
    std::map<uint256, std::vector<unsigned char>>& out)
{
    DataStream prefix;
    prefix << DBKeys::PRICOIN_SWAP_CEREMONY;
    std::unique_ptr<DatabaseCursor> cursor = m_batch->GetNewPrefixCursor(prefix);
    if (!cursor) return false;
    while (true) {
        DataStream ssKey;
        ...
        std::string type;
        ssKey >> type;
        if (type != DBKeys::PRICOIN_SWAP_CEREMONY) continue;
        uint256 ceremony_id;
        ssKey >> ceremony_id;
        std::vector<unsigned char> blob;
        ssValue >> blob;
...
bool WalletBatch::WritePricoinAdaptorSwap(const uint256& swap_id,
                                            const std::vector<unsigned char>& blob)
{
    return WriteIC(std::make_pair(DBKeys::PRICOIN_ADAPTOR_SWAP, swap_id), blob);
}

bool WalletBatch::ErasePricoinAdaptorSwap(const uint256& swap_id)
{
    return EraseIC(std::make_pair(DBKeys::PRICOIN_ADAPTOR_SWAP, swap_id));
}

bool WalletBatch::ReadAllPricoinAdaptorSwaps(
    std::map<uint256, std::vector<unsigned char>>& out)
{
    DataStream prefix;
    prefix << DBKeys::PRICOIN_ADAPTOR_SWAP;
    std::unique_ptr<DatabaseCursor> cursor = m_batch->GetNewPrefixCursor(prefix);
    if (!cursor) return false;
    while (true) {
        DataStream ssKey;
        ...
        std::string type;
        ssKey >> type;
        if (type != DBKeys::PRICOIN_ADAPTOR_SWAP) continue;
        uint256 sid;
        ssKey >> sid;
        std::vector<unsigned char> blob;
        ssValue >> blob;
        out.emplace(sid, std::move(blob));
    }
    return true;
}

bool WalletBatch::WritePricoinBtcMusig2Nonce(const uint256& digest,
                                               const std::vector<unsigned char>& blob)
{
    return WriteIC(std::make_pair(DBKeys::PRICOIN_BTC_MUSIG2_NONCE, digest), blob);
}

bool WalletBatch::ErasePricoinBtcMusig2Nonce(const uint256& digest)
{
    return EraseIC(std::make_pair(DBKeys::PRICOIN_BTC_MUSIG2_NONCE, digest));
}

bool WalletBatch::ReadAllPricoinBtcMusig2Nonces(
    std::map<uint256, std::vector<unsigned char>>& out)
{
    DataStream prefix;
    prefix << DBKeys::PRICOIN_BTC_MUSIG2_NONCE;
    std::unique_ptr<DatabaseCursor> cursor = m_batch->GetNewPrefixCursor(prefix);
    if (!cursor) return false;
    while (true) {
        DataStream ssKey;
        ...
        std::string type;
        ssKey >> type;
        if (type != DBKeys::PRICOIN_BTC_MUSIG2_NONCE) continue;
        uint256 digest;
        ssKey >> digest;
        std::vector<unsigned char> blob;
        ssValue >> blob;
...
bool WalletBatch::WritePricoinOffer(const uint256& order_id,
                                      const std::vector<unsigned char>& blob)
{
    return WriteIC(std::make_pair(DBKeys::PRICOIN_OFFER, order_id), blob);
}

bool WalletBatch::ErasePricoinOffer(const uint256& order_id)
{
    return EraseIC(std::make_pair(DBKeys::PRICOIN_OFFER, order_id));
}

bool WalletBatch::WritePricoinChainWatch(const uint256& digest,
                                           const std::vector<unsigned char>& blob)
{
    return WriteIC(std::make_pair(DBKeys::PRICOIN_CHAIN_WATCH, digest), blob);
}

bool WalletBatch::ErasePricoinChainWatch(const uint256& digest)
{
    return EraseIC(std::make_pair(DBKeys::PRICOIN_CHAIN_WATCH, digest));
}

bool WalletBatch::ReadAllPricoinChainWatches(
    std::map<uint256, std::vector<unsigned char>>& out)
{
    DataStream prefix;
    prefix << DBKeys::PRICOIN_CHAIN_WATCH;
    std::unique_ptr<DatabaseCursor> cursor = m_batch->GetNewPrefixCursor(prefix);
    if (!cursor) return false;
    while (true) {
        DataStream ssKey;
        ...
        std::string type;
        ssKey >> type;
        if (type != DBKeys::PRICOIN_CHAIN_WATCH) continue;
        uint256 digest;
        ssKey >> digest;
        std::vector<unsigned char> blob;
        ssValue >> blob;
...
bool WalletBatch::ReadAllPricoinOffers(
    std::map<uint256, std::vector<unsigned char>>& out)
{
    DataStream prefix;
    prefix << DBKeys::PRICOIN_OFFER;
    std::unique_ptr<DatabaseCursor> cursor = m_batch->GetNewPrefixCursor(prefix);
    if (!cursor) return false;
    while (true) {
        DataStream ssKey;
        ...
        std::string type;
        ssKey >> type;
        if (type != DBKeys::PRICOIN_OFFER) continue;
        uint256 oid;
        ssKey >> oid;
        std::vector<unsigned char> blob;
        ssValue >> blob;
        out.emplace(oid, std::move(blob));
    }
    return true;
}
